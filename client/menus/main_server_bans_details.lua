...
function main_server_bans_details_showContentThisFrame(playerGroup)
	...
	RageUI.Separator(
		_U(
			...
			_var.bans.selectedBan.type == "BAN"
					and _U("main_server_bans_unit_valid")
	...
	RageUI.Separator(
		_U(
			"main_server_bans_details_date",
			timeFormat(_U("date_format_long"), datas.writeDetails)
		)
	)
	RageUI.Separator(_U("main_server_bans_details_time", datas.duration))
	RageUI.Separator(
		_U(
			"main_server_bans_details_expiration",
			timeFormat(_U("date_format_long"), datas.expirationDetails)
		)
	)
	RageUI.ButtonWithStyle(
		...
		Config.Groups[playerGroup].Access["submenu_server_bans_edit"]
