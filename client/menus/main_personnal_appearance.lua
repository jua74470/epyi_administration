...
function main_personnal_appearance_showContentThisFrame(playerGroup)
    RageUI.ButtonWithStyle(_U("main_personnal_appearance_back"), _U("main_personnal_appearance_back_desc"), {}, true, function(_h, _a, Selected)
        if Selected then
    ...
    RageUI.List(_U("main_personnal_appearance_peds_filter"), _var.menu.pedsCategoryFilterArray, _var.menu.pedsCategoryFilterArrayIndex, _U("main_personnal_appearance_peds_filter_desc"), {}, true, function(_h, _a, _s, Index)
    ...
    for _k, category in pairs(_var.peds.list) do
        if _var.menu.pedsCategoryFilterArray[_var.menu.pedsCategoryFilterArrayIndex] == category.category or _var.menu.pedsCategoryFilterArray[_var.menu.pedsCategoryFilterArrayIndex] == _U("main_personnal_appearance_pedscategory0") then
            for _k, model in pairs(category.models) do
                RageUI.ButtonWithStyle(model.label .. " ~m~→ ~c~" .. model.model, _U("main_personnal_appearance_peds_desc", model.label, model.model), { RightBadge = RageUI.BadgeStyle.Clothes }, not _var.menus.admin.cooldowns.items, function(_h, _a, Selected)
                    if Selected then
