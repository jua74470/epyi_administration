...
function main_vehicles_current_showContentThisFrame(playerGroup)
	...
	RageUI.Checkbox(_U("main_vehicles_current_engine"), _U("main_vehicles_current_engine_desc"), GetIsVehicleEngineRunning(pedVehicle), { Enabled = Config.Groups[playerGroup].Access["submenu_vehicles_current_engine"] }, function() end, function()
		if not pedVehicle or GetPedInVehicleSeat(pedVehicle, -1) ~= ped then
			ESX.ShowNotification(_U("self_not_in_vehicle"))
			return
		end
	...
	RageUI.Checkbox(_U("main_vehicles_current_freeze"), _U("main_vehicles_current_freeze_desc"), IsEntityPositionFrozen(pedVehicle), { Enabled = Config.Groups[playerGroup].Access["submenu_vehicles_current_freeze"] }, function() end, function()
		if not pedVehicle or GetPedInVehicleSeat(pedVehicle, -1) ~= ped then
			ESX.ShowNotification(_U("self_not_in_vehicle"))
			return
		end
