	...
	RageUI.ButtonWithStyle(
		...
		function(_h, _a, Selected)
			if Selected then
				if
					not pedVehicle
					or GetPedInVehicleSeat(pedVehicle, -1) ~= ped
				then
				...
				if GetResourceState("VehicleDeformation") == "started" then
					TriggerServerEvent(
						"VehicleDeformation:fix_sv",
						NetworkGetNetworkIdFromEntity(pedVehicle)
					)
				end
	...
	RageUI.List(
		...
		function(_h, _a, Selected, Index)
			...
			if Selected then
				...
				if
					_var.vehicle.doorArray[_var.vehicle.doorArrayIndex]
					== _("door_frontleft")
				then
					local doorState = GetVehicleDoorAngleRatio(pedVehicle, 0)
						< 0.1
				...
				elseif
					...
					== _("door_frontright")
				then
					local doorState = GetVehicleDoorAngleRatio(pedVehicle, 1)
						< 0.1
					if not doorState then
				...
				elseif
					...
					== _("door_backleft")
				then
					local doorState = GetVehicleDoorAngleRatio(pedVehicle, 2)
						< 0.1
					if not doorState then
				...
				elseif
					...
					== _("door_backright")
				then
					local doorState = GetVehicleDoorAngleRatio(pedVehicle, 3)
						< 0.1
					if not doorState then
				...
				elseif
					...
					== _("door_hood")
				then
					local doorState = GetVehicleDoorAngleRatio(pedVehicle, 4)
						< 0.1
					if not doorState then
				...
				elseif
					...
					== _("door_trunk")
				then
					local doorState = GetVehicleDoorAngleRatio(pedVehicle, 5)
						< 0.1
					if not doorState then
	...
	RageUI.List(
		...
		function(_h, _a, Selected, Index)
			...
			if Selected then
				...
				_threads.vehicleboost.enable(
					pedVehicle,
					_var.vehicle.boostArrayIndex
				)
				ESX.ShowNotification(
					_U(
						"notif_vehicleboost_success",
						_var.vehicle.boostArray[_var.vehicle.boostArrayIndex]
					)
				)
			end
		end
