...
function main_server_showContentThisFrame(playerGroup)
	if GetResourceState("cd_easytime") == "started" then
		RageUI.List(
			...
			{},
			Config.Groups[playerGroup].Access["submenu_server_weather"]
		...
		RageUI.List(
			...
			{},
			Config.Groups[playerGroup].Access["submenu_server_time"]
				and not _var.menus.admin.cooldowns.items,
			function(_h, _a, Selected, Index)
		...
		RageUI.Checkbox(
			...
			function()
				if
					not Config.Groups[playerGroup].Access["submenu_server_blackout"]
				then
					return
				end
	...
	RageUI.ButtonWithStyle(
		...
		{ RightLabel = "→" },
		Config.Groups[playerGroup].Access["submenu_server_clearall"]
			and not _var.menus.admin.cooldowns.items,
		function(_h, _a, Selected)
			if Selected then
				Citizen.CreateThread(function()
					...
					local vehicles =
						ESX.Game.GetVehiclesInArea(pedCoords, radius)
	...
	RageUI.ButtonWithStyle(
		...
		{ RightLabel = "→" },
		Config.Groups[playerGroup].Access["submenu_server_clearvehicles"]
			and not _var.menus.admin.cooldowns.items,
		function(_h, _a, Selected)
			if Selected then
				Citizen.CreateThread(function()
	...
	RageUI.ButtonWithStyle(
		...
		{ RightLabel = "→" },
		Config.Groups[playerGroup].Access["submenu_server_clearpeds"]
			and not _var.menus.admin.cooldowns.items,
		function(_h, _a, Selected)
			if Selected then
				Citizen.CreateThread(function()
	...
	RageUI.ButtonWithStyle(
		...
		{ RightLabel = "→" },
		Config.Groups[playerGroup].Access["submenu_server_bans"]
			and not _var.menus.admin.cooldowns.items,
