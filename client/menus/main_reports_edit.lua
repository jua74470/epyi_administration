...
function main_reports_edit_showContentThisFrame(playerGroup)
	...
	if
		Config.Groups[_var.reports.list[_var.reports.selectedReport].user.group]
		~= nil
	then
	...
	RageUI.Separator(
		_U(
			...
			(
				_var.reports.list[_var.reports.selectedReport].staff.taken
					and _U("main_reports_edit_status_taken")
	...
	if _var.reports.list[_var.reports.selectedReport].staff.taken then
		if
			_var.reports.list[_var.reports.selectedReport].staff.takerIdentifier
				~= nil
			and _var.reports.list[_var.reports.selectedReport].staff.takerSource
				~= nil
		then
			local takerGroup = _U("invalid")
			if
				Config.Groups[_var.reports.list[_var.reports.selectedReport].staff.takerGroup]
				~= nil
			then
	...
	if not _var.reports.list[_var.reports.selectedReport].staff.taken then
		RageUI.ButtonWithStyle(
			...
			_U(
				"main_reports_report_desc",
				_var.reports.list[_var.reports.selectedReport].user.reason
			),
			{},
			...
			function(_h, _a, Selected)
				if Selected then
					...
					editedReport.staff.takerIdentifier =
						_var.client.playerData.identifier
					editedReport.staff.takerSource =
	...
	RageUI.ButtonWithStyle(
		...
		function(_h, _a, Selected)
			if Selected then
				_var.client.playerData = ESX.GetPlayerData()
				_var.players.list = GlobalState["epyi_administration:playerList"]
					or {}
				for _k, player in pairs(_var.players.list) do
					if
						player.identifier
						== _var.reports.list[_var.reports.selectedReport].user.identifier
					then
