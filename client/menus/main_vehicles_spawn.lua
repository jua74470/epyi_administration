...
function main_vehicles_spawn_showContentThisFrame(playerGroup)
		...
		function(_h, _a, Selected)
			if Selected then
				local search = textEntry(
					_U("textentry_search_vehicle_model"),
					_var.menu.searchVehicle,
					30
				)
	...
	for _k, vehicle in pairs(_var.vehicle.list) do
		_var.menu.findedVehicle = string.find(
			vehicle.label,
			tostring((_var.menu.searchVehicle):gsub("^%l", string.upper))
		)
		if _var.menu.findedVehicle then
			RageUI.ButtonWithStyle(
				vehicle.label .. " ~c~[" .. string.upper(vehicle.type) .. "]",
				_U(
					"main_vehicles_spawn_spawn_desc",
					vehicle.label,
					vehicle.type
				),
					...
					if Selected then
						...
						ESX.Game.SpawnVehicle(
							...
							function(callback_vehicle)
								...
								if
									GetResourceState("cd_garage") == "started"
								then
									Wait(100)
									TriggerEvent(
										"cd_garage:AddKeys",
