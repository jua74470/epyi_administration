...
function main_players_showContentThisFrame(playerGroup)
	...
	RageUI.ButtonWithStyle(
		...
		{
			RightLabel = (
				_var.menu.playersFilter ~= "" and _var.menu.playersFilter
				or _U("no_filter")
			),
		},
		true,
	...
	for _k, player in pairs(_var.players.list) do
		...
		then
			...
			if
				...
				and Config.Groups[player.group].Priority
			...
			RageUI.ButtonWithStyle(
				...
				Config.Groups[playerGroup].Access["submenu_players_interact"]
					and (
						Config.Groups[playerGroup].Access["submenu_players_interact_highergroup"]
							and true
						or targetisLower
					),
				function(_h, _a, Selected)
