...
function main_vehicles_favorites_showContentThisFrame(playerGroup)
	...
	RageUI.ButtonWithStyle(_U("main_vehicles_favorites_add"), _U("main_vehicles_favorites_add_desc"), { RightLabel = "→" }, true, function(_h, _a, Selected)
		if Selected then
	...
	for id, content in pairs(_var.datas.list) do
		if content.type == "FAVVEH" and content.owner == _var.client.playerData.identifier then
			...
			RageUI.List(_datas.vehicleName .. " (" .. GetDisplayNameFromVehicleModel(_datas.model) .. ")", _var.menu.favritesActionsArray, _var.menu.favritesActionsArrayIndex, _U("main_vehicles_favorites_interact_desc", _datas.vehicleName), {}, true, function(_h, _a, Selected, Index)
