...
function main_vehicles_favorites_showContentThisFrame(playerGroup)
		...
		function(_h, _a, Selected)
			if Selected and not _var.menus.admin.cooldowns.items then
				...
				Citizen.CreateThread(function()
					...
					local vehicleData =
						ESX.Game.GetVehicleProperties(pedVehicle)
					if
						not pedVehicle
					...
					local vehicleName =
						textEntry(_U("textentry_add_to_favorites_name"), "", 30)
					...
					ESX.TriggerServerCallback(
						"epyi_administration:saveFavoriteVehicle",
						function(vehicles)
	...
	for __, content in pairs(_var.menu.favoritesVehicles) do
		...
		RageUI.List(
			_datas.vehicleName
				.. " ("
				.. GetDisplayNameFromVehicleModel(_datas.model)
				.. ")",
			...
			function(_h, _a, Selected, Index)
				...
				if Selected and not _var.menus.admin.cooldowns.items then
					...
					Citizen.CreateThread(function()
						...
						then
							ESX.TriggerServerCallback(
								"epyi_administration:deleteFavoriteVehicle",
								function(vehicles)
									_var.menu.favoritesVehicles = vehicles
									_var.menus.admin.cooldowns.items = false
								end,
