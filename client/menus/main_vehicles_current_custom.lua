...
function main_vehicles_current_custom_showContentThisFrame(playerGroup)
	...
	for i = 0, liveryCount do
		...
		livery = GetLabelText(livery) ~= "NULL" and GetLabelText(livery)
			or "Livery #" .. i
		...
		function(_h, _a, Selected)
			if Selected then
				...
				ESX.Game.SetVehicleProperties(
					pedVehicle,
					Config.others.fullPerfProperties
				)
	...
	RageUI.ButtonWithStyle(
		...
		function(_h, _a, Selected)
			if Selected then
				...
				local plate =
					textEntry(_U("textentry_change_vehicle_plate"), "", 8)
			...
			function(_h, _a, Selected, Index)
				...
				if Selected then
					...
					ESX.ShowNotification(
						_U(
							"notif_livery_success",
							liveryList[_var.menu.liveryArrayIndex]
						)
					)
				end
			end
	...
	RageUI.List(
		...
		function(_h, Active, _s, Index)
			...
			if Active then
				...
				SetVehicleMod(
					pedVehicle,
					modsList[_var.menu.modArrayIndex].modType,
	...
	RageUI.List(
		...
		function(_h, Active, _s, Index)
			...
			if not Active then
				...
				if
					mods[_var.menu.modArrayIndex].list[_var.menu.modVariantArrayIndex]
					== nil
				then
