...
function main_server_bans_showContentThisFrame(playerGroup)
	...
	RageUI.ButtonWithStyle(
		...
		{
			RightLabel = (
				_var.menu.bansFilter ~= "" and _var.menu.bansFilter
				or _U("no_filter")
			),
		},
		true,
		function(_h, _a, Selected)
			if Selected then
				local search =
					textEntry(_U("textentry_search"), _var.menu.bansFilter, 30)
		...
		if content.type == "BAN" or content.type == "D_BAN" then
			if
				...
				or string.find(
						...
						string.lower(_var.menu.bansFilter)
					)
					~= nil
			then
				count = count + 1
				RageUI.ButtonWithStyle(
					...
					{
						RightLabel = content.type == "BAN" and _U(
							"main_server_bans_unit_valid"
						) or _U("main_server_bans_unit_finished"),
					},
					true,
					function(_h, _a, s)
